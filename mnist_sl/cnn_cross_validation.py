...
# train function
def train(epochs, model, trainloader, optim, criterion):
    model.train()
...
if __name__ == '__main__':
    ...
    total_dataset = ConcatDataset([SLDataset(transform=transforms, train=True), test_data])

    testloader = DataLoader(test_data, batch_size=16, shuffle=True)

    PATH = './models/cross_validation_cnn.pth'
    model = CNN()
    ...
    k_folds_cross_validation(4, total_dataset)
